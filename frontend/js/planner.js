...
function loadProductivityAnalytics() {
    ...
    const subjectsCovered =
        document.getElementById("subjectsCovered");
    ...
    if (!window.plannerTasks) return;

    const totalTasks = window.plannerTasks.length;

    const completedTasks =
        window.plannerTasks.filter(task => task.completed).length;

    // Calculate Study Hours

    let totalHours = 0;

    window.plannerTasks.forEach(task => {

        if (!task.start_time || !task.end_time) return;

        const start = task.start_time.split(":");
        const end = task.end_time.split(":");
    ...
    // Count unique study dates

    const uniqueDays = new Set();

    window.plannerTasks.forEach(task => {

        uniqueDays.add(task.study_date);
